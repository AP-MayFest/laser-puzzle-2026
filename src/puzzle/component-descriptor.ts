...
const encodeCardinalDirection = (direction: NormalizedVec2): CardinalDirection => {
    ...
    const [, argmax] = similarity.reduce<[number, number]>(([v, i], w, j) => w > v ? [w, j] : [v, i], [-Infinity, -1]);
    return cardinalDirections[argmax];
};
export const assertingCardinalDirection = (direction: unknown): CardinalDirection => {
    if ((cardinalDirections as readonly unknown[]).includes(direction)) return direction as CardinalDirection;
...
export const assertingDiagonalDirection = (direction: unknown): DiagonalDirection => {
    if ((diagonalDirections as readonly unknown[]).includes(direction)) return direction as DiagonalDirection;
...
export const assertingPolarity = (polarity: unknown): Polarity => {
    if ((polarities as readonly unknown[]).includes(polarity)) return polarity as Polarity;
...
export const createComponent = (descriptor: ComponentDescriptor): Component => {
    switch (descriptor.kind) {
        ...
        case 'target': return new Target(decodeCardinalDirection(descriptor.direction), descriptor.colors.map(colorToWaveLength) as [WaveLength] | [WaveLength, WaveLength]);
...
export const describeComponent = (component: Component): ComponentDescriptor => {
    ...
    if (component instanceof Target) return { kind: 'target', direction: encodeCardinalDirection(component.direction), colors: component.waveLengthList.map(waveLengthToColor) as [Color] | [Color, Color]};
