...
const colorDistinctionProblem: Problem = {
    board: { width: 5, height: 5, placements: [
        { position: new Vec2(1, 3), descriptor: { kind: 'laser', color: 'R', direction: 'E' } },
    ...
    reserve: { doubleSidedMirror: 1 },
};

const dichroicMirrorProblem: Problem = {
    board: { width: 7, height: 7, placements: [
    ...
    reserve: { dichroicMirror: 1 },
};

const polarityProblem: Problem = {
    board: { width: 7, height: 7, placements: [
    ...
    reserve: { polarizerD: 1 },
};

const pbsProblem: Problem = {
    board: { width: 5, height: 5, placements: [
        { position: new Vec2(1, 3), descriptor: { kind: 'laser', color: 'R', direction: 'E' } },
...
new Runtime('canvas-core-concept', coreConceptProblem, scrollTo('color-distinction')).start();
new Runtime('canvas-color-distinction', colorDistinctionProblem, scrollTo('dichroic-mirror')).start();
new Runtime('canvas-dichroic-mirror', dichroicMirrorProblem, scrollTo('polarity')).start();
new Runtime('canvas-polarity', polarityProblem, scrollTo('pbs')).start();
new Runtime(
    'canvas-pbs',
    pbsProblem,
    () => {
